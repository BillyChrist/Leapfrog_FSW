 ...
 * Description: Cross-platform UART communication wrapper for POSIX systems.
 *              Provides thread-safe serial I/O with optional delimiter or
 *              fixed-length packet modes. Supports background receive queuing.
 *
 * Reference  : https://blog.mbedded.ninja/electronics/communication-protocols/uart-communication-protocol/
 * Notes      : - `num_bytes` controls fixed-packet mode; if -1, delimiter is used.
 *              - Queue size limits number of buffered receive packets.
 *
 * License    : MIT (or internal use — update if needed)
 *===================================================================*/
...
public:
    ...
    // \param serial communication delimiter
    ...
    bool Send(vector<uint8_t>, bool = true);
